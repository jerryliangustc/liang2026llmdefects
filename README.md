...
本项目从论文《大语言模型高级逻辑与语义缺陷研究：构造范式、根源剖析与评测基准扩展》（梁家瑞，2026-05-27）中抽取并整理了 **12 道高阶逻辑/语义陷阱题**，形成一个可复现的最小评测基准（**Benchmark**）。论文原稿（LaTeX）保留在本仓库根目录，全部题目与答案整理在 [`benchmark/questions.md`](benchmark/questions.md)。
...
受成本与网络限制，本实验主要使用国产模型，共报告三个模型的结果。合计得分列与论文附录原表一致（DeepSeek 5/12、豆包 5/12、ChatGPT 7/12，正确率 41.7%、41.7%、58.3%）。
...
└── paper-raw/         #（可选）存放论文 LaTeX 源文件
```

当前根目录直接保留论文 `.tex` 原稿（文件名含中文）。若希望仓库更整洁，可将该文件移入 `paper-raw/` 目录，不影响任何链接。
